...
def _fresh_import():
    """Re-import web_search to avoid cached module state between tests."""
    import mao.core.web_search as mod
...
def test_web_search_brave_succeeds() -> None:
    ...
    with patch.dict(os.environ, {"BRAVE_API_KEY": "test-key"}), \
        ...
        mod = _fresh_import()
        results = mod.web_search("Alzheimer treatment", num_results=3)

    assert len(results) == 1
    assert results[0]["title"] == "AD Treatment"
    assert results[0]["href"] == "http://example.com"
...
def test_web_search_falls_back_to_ddg() -> None:
    ...
    with patch.dict(os.environ, env_without_keys, clear=True), \
        ...
        mod = _fresh_import()
        results = mod.web_search("test query")
...
def test_web_search_brave_skipped_silently_when_no_key() -> None:
    ...
    with patch.dict(os.environ, env_without_keys, clear=True), \
         patch("mao.core.web_search._ddg_search", return_value=fake_ddg):
        mod = _fresh_import()
        results = mod.web_search("no key test")
...
def test_web_search_serpapi_used_when_brave_missing() -> None:
    ...
    with patch.dict(os.environ, env, clear=True), \
        ...
        mod = _fresh_import()
        results = mod.web_search("serp query", num_results=2)
